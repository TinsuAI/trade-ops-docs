...
TradeOps is for **customs brokers** that serve many export-facing enterprises. The first customer is Trọng Tín. The end-clients of the broker are DN chế xuất and SXXK enterprises whose customs declarations, CO dossiers, and BCQT settlement reports all flow through the broker.
...
A customs broker like Trọng Tín runs a multi-client operations factory:
...
3. **Multi-year dossier interconnections are fragile.** Last year's TKNK feeds this year's BCQT. The BOM used to support a CO must reconcile against BCQT later. One mistake or one drift breaks the whole chain — sometimes years after it was made.
4. **Audit reconstruction is expensive.** The most common audit trigger is **kiểm tra sau thông quan** (post-clearance audit) — customs can reopen and query a dossier within roughly 5 years of clearance, often at the enterprise's premises. Rebuilding the chain of supporting documents takes days. Sometimes the chain cannot be rebuilt — because the staff member who built it has moved on. The ability to defend a dossier years later is what separates a confident broker from an exposed one.
...
- **Multiple inventory views diverge silently.** Physical, accounting, HQ-declared, and CO consumption all differ — sometimes intentionally — but no system holds them side by side or reconciles them.
...
- A NVL declared with mã hàng "X-1234" on a 2023 TKNK appears under mã hàng "X-1234A" on a 2024 TKNK after a naming clarification (mã hàng on the declaration line is enterprise-controlled and is what drifts in practice; the underlying mã HS classification typically stays stable). The ERP system kept calling the same physical item "INT-001" both years. By 2025, the BCQT reconciliation cannot match accounting consumption to the customs balance without manual code mapping.
